...
class Parser {
	 ...
	 *
	 * @config string empty_element_suffix Set to ">" for HTML output
	 * @config integer tab_width Number of space for each tab
	 */
	...
	public function setConfig(array $config) {
		...
		foreach ($config_list as $config_name) {
			if (isset($config[$config_name])) {
				$this->{$config_name} = $config[$config_name];
			}
		}
	}
...
//				}
	...
	protected function unhash($text) {
		$html_hashes = $this->html_hashes;

		return preg_replace_callback(
			'/(.)\x1A[0-9]+\1/', 
			function($matches) use ($html_hashes) {
	...
	public static function factory($type = "") {
		...
		$parser_class = "Markdown\\Parser$type";
